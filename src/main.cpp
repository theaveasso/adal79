...
#define DATA_DIR std::string(CURRENT_DIR) + "/data"
...
int main(int argc, char** argv)
{
    ...
    adl::init_cb game_init = [&]() {
        try
        {
            auto              level_path = fmt::format("{}/level.dat", std::string(DATA_DIR));
            auto              file       = make_shared<adl::file_handler>(level_path);

            adl::level_parser lp(reg, file);
            lp.parse();
        }
